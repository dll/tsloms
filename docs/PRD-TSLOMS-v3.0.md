...
- 影像底图：默认 **OpenStreetMap**，**可切换高德 / 百度实景底图**（已接入高德 style8路网、百度瓦片，均做 WGS84→GCJ-02/BD-09 坐标转换；AK 复用 WXX 项目）。
- **地图操作工具**：内置 Cesium 搜索/主页/全屏/导航帮助/比例尺控件 + 自定义 放大/缩小/复位视角/全览设备/全屏 工具条；大屏全屏布局（地图窗口最大化）。
- 点位联动下钻（见 §5.9）。
- **点位联动下钻**：点击地图设备点位（或侧栏列表）弹出下钻面板，聚合展示该设备**派单参考**（活跃故障/工单）、**维修耗材**（库存与预警）、**监控/视频**，并可一键跳转「视频与监控」「问题反馈」面板。
