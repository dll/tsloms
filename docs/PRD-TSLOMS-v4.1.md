...
**适用场景**：城市交通信号灯设备 MQTT 通信对接、故障自动研判、维修工单流转、**维修成本（费用/库存/采购/供应商）归集**、运维数据可视化统计、AI 故障预测/诊断/生命周期溯源、固件 OTA 升级管理
...
**版本说明**：V4.1 为**正式发布版**。在 V4.0 基础上完成**全功能/全角色/多端 结项复核**，并在系统治理章节补充**三角色权限矩阵**、明确**多端应用定位**（需求方裁定：仅交付后端+Web 管理端，不做多端；预留对接接口）。以 PRD V2.0 与《设备协议确认清单》为业务范围底盘，协议 PDF 与故障 DOCX 为权威依据。**本版作为最终发布基线。**
...
- AI 域：`ai_config`、`ai_usage`、`ai_predictions`
...
| 9 | 系统治理 | 三角色、强密码、审计、离线判定 |
| 10 | 测试 | Go 全量测试绿、前端 vue-tsc/ESLint/build 通过 |
| 11 | 角色与多端 | 三角色权限矩阵实测通过；多端定位明确（Web 单端+预留对接） |
...
| 文档 | 路径 |
|------|------|
| 本需求文档 V4.1 | `docs/PRD-TSLOMS-v4.1.md` |
| 结项审核报告 V4.1 | `docs/SAR-TSLOMS-v4.1.md` |
| 用户操作手册 V1.0 | `docs/TSLOMS操作手册-v1.0.md` |
