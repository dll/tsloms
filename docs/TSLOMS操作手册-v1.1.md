...
## 8. AI 化功能（主动巡检 + 流程 Copilot）

> 平台 AI 化按**六级演进路线**推进，当前已到 **L4 流程 Copilot**（详见 `docs/PRD-TSLOMS-v4.3.md`）：
> L1 基础 AI 功能 → L2 AI 原生增强（报告/建议）→ L3 AI 主动服务（定时巡检+推送）→ L4 流程 Copilot 嵌入 → L5 自然语言交互 → L6 自主决策。
...
**Q19：怎么调用故障/工单 AI 建议？**
- 处理：进入「故障管理」/「工单管理」，对单条故障或工单点击对应的「AI 建议 / 派单辅助 / Copilot」按钮，系统展示 AI 生成的摘要、优先级、应对预案与备件预领（再次确认可刷新）。
...
| GET | /api/v1/ai/advice/fault/:id | 故障处置 AI 建议（Copilot） | 读 |
| GET | /api/v1/ai/advice/workorder/:id?stage=summary | 工单 Copilot（维修小结） | 读 |
...
| GET | /api/v1/ai/advice/fault/:id | 故障 AI 建议（派单辅助） | 读 |
| GET | /api/v1/ai/advice/workorder/:id | 工单 Copilot（copilot/summary） | 读 |
