...
> 平台 AI 化按**六级演进路线**推进，当前已达 **L4 流程 AI 辅助全流程覆盖**（详见 `docs/PRD-TSLOMS-v4.3.md`）：
> L1 基础 AI 功能 → L2 AI 原生增强（报告/建议）→ L3 AI 主动服务（定时巡检+推送）→ L4 流程 AI 辅助嵌入（全操作表单）→ L5 自然语言交互 → L6 自主决策。
