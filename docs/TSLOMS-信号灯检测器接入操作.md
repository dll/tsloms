...
| 检测器账号 | 在「检测器接入 → 真实硬件接入」页面点击“生成检测器 MQTT 账号”创建；账号和密码不写入仓库、不在文档明文保存 |

> 检测器使用**独立账号**，避免与服务端共用 `tsloms` 凭据，便于按设备审计与吊销。
> 推荐方式：登录 TSLOMS 后进入「检测器接入 → 真实硬件接入」，点击“生成检测器 MQTT 账号”。系统生成随机账号和密码并调用 EMQX `built_in_database` 创建；密码只在当前页面显示一次，复制后配置到检测器。服务器需预先配置 `EMQX_API_TOKEN`（或 `EMQX_API_USERNAME` + `EMQX_API_PASSWORD`），普通操作人员不需要 SSH。
>
> 运维备用方式（仅系统管理员）：
