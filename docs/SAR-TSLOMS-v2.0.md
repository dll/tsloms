...
| 需求符合性 | 🟢 除硬件相关外已实现 | 核心+用户管理+离线判定+看板+路口管理+地图大屏已实现；固件/配置下发/其余可选扩展留待硬件 |
...
- ✅ **地图大屏**：`/map` 路由，ECharts geo + 设备经纬度在线/离线打点（无地图 AK 依赖）
...
| 6 | ✅ **设备地图可视化** | 路口设备地图分布 | 已用 ECharts geo 实现（无 AK）；如需百度/高德实景图层 | 已实现（实景图层可选） |
