...
| 需求符合性 | 🟡 除硬件相关外已实现 | 核心+用户管理+离线判定+看板增强已实现；固件/配置下发/6可选扩展留待硬件 |
...
### 4.3 未实现（后续迭代 / 硬件决定）
固件 OTA、配置下发（CMD_UPDATE_CONFIG）、远程重启、MQTT 异步化、报文日志分区（以上见 PRD V3 §9）；六项硬件可选扩展见「七之附」。
...
| 6 | 🔴 **设备地图可视化** | 路口设备地图分布 | 需地图服务（百度/高德 AK）与设备经纬度 | 可选（未启动） |
