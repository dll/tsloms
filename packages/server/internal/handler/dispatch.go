...
func DispatchReference(c *gin.Context) {
	...
	model.DB.Where("device_hw_id = ? AND status IN ?", hwID, []string{
	...
	model.DB.Where("device_hw_id = ? AND status IN ?", hwID, []string{model.WorkOrderStatusPending, model.WorkOrderStatusProcessing}).
		Order("created_at DESC").Find(&orders)

	// 耗材（设备绑定物料，来自统一库存模块）
	var materials []model.Material
	model.DB.Where("device_hw_id = ?", hwID).Find(&materials)

	// 媒体（举证/监控/时间视频）
	var media []model.DeviceMedia
	model.DB.Where("device_hw_id = ?", hwID).Order("created_at DESC").Limit(20).Find(&media)
