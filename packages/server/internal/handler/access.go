...
func CreateMQTTDeviceCredential(c *gin.Context) {
	...
	if token == "" && cfg.EMQXAPIUsername != "" && cfg.EMQXAPIPassword != "" {
		...
		if err != nil {
			serverError(c, fmt.Errorf("EMQX 管理 API 登录失败: %w", err))
			return
		}
	}
	if token == "" {
...
func randomCredential(length int) string {
	...
	for i := range b {
		...
		b[i] = credentialAlphabet[int(x[0])%len(credentialAlphabet)]
	}
	return string(b)
}
...
func emqxCreateUser(base, token, username, password string) error {
	...
	req, err := http.NewRequest(http.MethodPost, strings.TrimRight(base, "/")+"/api/v5/authentication/password_based:built_in_database/users", strings.NewReader(string(body)))
	if err != nil {
		return err
	}
