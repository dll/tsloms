...
func buildEventPak(records []EventRecord) []byte {
	...
	_tmp := make([]byte, 2)
	binary.BigEndian.PutUint16(_tmp, uint16(len(records)))
	buf = append(buf, _tmp...)
	binary.BigEndian.PutUint16(_tmp, uint16(len(recBytes)))
	buf = append(buf, _tmp...)
...
func marshalEventRecords(records []EventRecord) []byte {
	...
	for _, r := range records {
		chunk := make([]byte, 24)
		binary.BigEndian.PutUint32(chunk[0:4], r.LedHwID)
		binary.BigEndian.PutUint32(chunk[4:8], r.SubHwID)
		binary.BigEndian.PutUint32(chunk[8:12], r.SwVer)
		binary.BigEndian.PutUint32(chunk[12:16], r.ConfVer)
		chunk[16] = uint8(r.LedState)
		chunk[17] = uint8(int8(r.ErrCode))
		binary.BigEndian.PutUint16(chunk[18:20], r.CurrentR)
		binary.BigEndian.PutUint16(chunk[20:22], r.CurrentY)
		binary.BigEndian.PutUint16(chunk[22:24], r.CurrentG)
