  ...
  <div class="access-page">
    ...
    <el-tabs v-model="tab" type="border-card">
      ...
      <el-tab-pane name="real">
        ...
        <ol>
          <li>在 <code>server/.env</code> 配置 <code>MQTT_BROKER</code>、<code>MQTT_USERNAME</code>、<code>MQTT_PASSWORD</code>（可选鉴权）。</li>
          <li>检测器按协议连接 Broker 并发布到 <code>trafficLight/{网络}/{站点}/{硬件ID}/U</code>。</li>
          <li>后台自动解析 CHECKIN/ALARM/POWER_ON → 研判 → 故障/工单，前端「故障管理」「设备管理」可见。</li>
          <li>时间同步/固件查询/固件升级应答由后台自动下发（/U → /D）。</li>
        </ol>
      </el-tab-pane>
...
import { Connection, Upload, Aim, Monitor } from '@element-plus/icons-vue'
